...
| DAU | 일자별 고유 사용자/세션 수 | UsageEvent |
| 회원가입 수 | 일자별 신규 회원 수 | User |
| 로그인 수 | 일자별 로그인 이벤트 수 | UsageEvent |
| 비회원/회원 이용 비율 | 일정 생성 요청 중 비회원/회원 비율 | TripPlan, UsageEvent |
...
| 일자별 일정 생성 수 | 날짜별 일정 생성 추이 | TripPlan |
| AI 호출 수 | 일정 생성/재생성/공유문구 생성 호출 수 | AiRequestLog |
| 인기 작품 | 일정 생성에 가장 많이 사용된 작품 | TripPlan.content_id |
| 인기 국가/도시 | 선택된 성지 기준 인기 지역 | PilgrimageSpot |
| 인기 성지 | 일정에 가장 많이 포함된 성지 | TripStop |
| 평균 여행 기간 | 생성 일정의 평균 duration_days | TripPlan |
| 예산 태그 분포 | LOW/NORMAL/HIGH 비율 | TripPlan |
| 저장 수 | 회원이 저장한 일정 수 | TripPlan.status |
